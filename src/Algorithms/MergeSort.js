...
const merge = (L, R, position, arraySteps, colorSteps) => {
	...
	while (L.length > 0 && R.length > 0) {
		if (L[A] < R[B]) {
			...
			//adding new step to the array after merging
			insertStep(arrayNew, position, arraySteps);
		}
		//only one element gets added to the arrayNew
		updateColor(position, colorSteps, arrayNew.length - 1, [], []);
	}


	if (L.length !== 0 || R.length !== 0) {
		//color the remaining elements that are going to add in the elements
...
const updateColor = (position, colorSteps, start, L, R) => {
	...
	if (end === start) {
		//if one element is there in the array then it is sorted , color it with red
		colorKey.fill(1, start, end + 1);
	} else {
		//else color it with blue
